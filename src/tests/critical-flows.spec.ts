...
test.describe("Critical User Flows", () => {
  test("Create account + onboarding", async ({ newUnonboardedPage }) => {
    ...
    await page
      .getByLabel(/About You/i)
      .fill("I grow organic strawberries in the valley.");
    await page.getByRole("button", { name: /Continue/i }).click();
  ...
  test("Create organization onboarding", async ({ newUnonboardedPage }) => {
    ...
    await expect(
      page.getByRole("cell", { name: "partner@garyfood.org" }),
    ).toBeVisible();

    await page
      .getByRole("button", { name: /Finish & Go to Dashboard/i })
      .click();
    await expect(page).toHaveURL(/.*\/org\/clients/);
  });

  test("Accept organization invitation", async ({
    ...
    await page.goto(
      `/verify-invite?org=${orgId}&code=${inviteRecord.code}&email=${email}`,
    );

    await expect(
      page.getByRole("heading", { name: /Join E2E Test Organization/i }),
    ).toBeVisible();
    ...
    await expect(page.getByLabel(/Invitation Code/i)).toHaveValue(
      inviteRecord.code,
    );
  ...
  test("Create listing", async ({ onboardedSellerPage }) => {
    ...
    await page
      .getByLabel(/Allow customers to set up recurring orders/i)
      .check();
  ...
  test("Subscribe", async ({ onboardedBuyerPage, sellerWithProducePage }) => {
    ...
    await page
      .locator("#email")
      .fill(`subscriber-test-${Date.now()}@example.com`);
    await page.locator("#cardNumber").fill("4242");
    await page.locator("#cardNumber").pressSequentially("4242424242424242");
    await page.locator("#cardExpiry").fill("12/28");
    await page.locator("#cardCvc").fill("123");
